...
if __name__ == "__main__":
    print("Testing GLM-4.1V Model with Fake Inputs")
    print("=" * 40)
    ...
    print(f"✓ Config: vision_dim={config.vision_config.hidden_size}, text_dim={config.text_config.hidden_size}")
    ...
    with torch.no_grad():
        ...
        print(f"✓ Text input {input_ids.shape} → logits {text_output.logits.shape}")
    
    # Test 2: Multimodal input
    print("\n--- Multimodal Forward Pass ---")
    
    # Create fake image patches
    patch_size = config.vision_config.patch_size
    spatial_merge = config.vision_config.spatial_merge_size
    patches_per_side = config.vision_config.image_size // patch_size // spatial_merge
    total_patches = patches_per_side ** 2
    
    pixel_values = torch.randn(total_patches, 3, patch_size, patch_size)
    image_grid_thw = torch.tensor([[1, patches_per_side, patches_per_side]])
    
    # Create input with image token
    text_before = torch.randint(10, 50, (3,))
    image_token = torch.tensor([config.image_token_id])  # Special image token
    text_after = torch.randint(10, 50, (3,))
    
    multimodal_ids = torch.cat([text_before, image_token, text_after]).unsqueeze(0)
    multimodal_mask = torch.ones_like(multimodal_ids)
    ...
    with torch.no_grad():
        ...
        print(f"✓ Multimodal input {multimodal_ids.shape} + {total_patches} patches → logits {multimodal_output.logits.shape}")
        print(f"  Image token at position: {(multimodal_ids == config.image_token_id).nonzero()[0, 1].item()}")
    
    print("\n" + "=" * 40)
    print("✓ GLM-4.1V Model Test Complete!")
    print(f"Key features: text+vision, {config.image_token_id}=image_token, 3D position encoding")
    print("Run with: python glm4v/modeling_glm4v.py")
